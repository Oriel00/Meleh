...
public partial class Month_StartEndTime : System.Web.UI.Page
{
    ...
    protected void DayPilotMonth1_BeforeEventRender(object sender, DayPilot.Web.Ui.Events.Month.BeforeEventRenderEventArgs e)
    {
        ...
        if (e.Start.TimeOfDay != TimeSpan.Zero || e.End.TimeOfDay != TimeSpan.Zero)
        {
            // start
            e.Areas.Add(new Area().Left(6).Top(3).Bottom(3).Width(35).Html(e.Start.ToString("HH:mm")).Visible());
            // end
            e.Areas.Add(new Area().Right(6).Top(3).Bottom(3).Width(35).Html(e.End.ToString("HH:mm")).Visible());
            // text
            e.Areas.Add(new Area().Left(46).Top(3).Bottom(3).Right(46).Html(e.Html).Visible());
            e.Html = " ";
        }
