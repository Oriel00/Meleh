<%@ Page Language="C#" MasterPageFile="~/Demo.master" AutoEventWireup="true" CodeFile="ScaleMinutes.aspx.cs"
    Inherits="Scheduler_ScaleMinutes" Title="Scale: Minutes (Scheduler) | DayPilot Pro for ASP.NET WebForms Demo"  %>
...
/* Event editing helpers - modal dialog */
	...
	function timeRangeSelected(start, end, resource) {
	    var modal = dialog();
	    modal.showUrl("New.aspx?time=yes&start=" + start.toStringSortable() + "&end=" + end.toStringSortable() + "&r=" + resource + "&hash=<%= PageHash %>");
	}
	
	function eventClick(e) {
	    var modal = dialog();
	    modal.showUrl("Edit.aspx?time=yes&id=" + e.value() + "&hash=<%= PageHash %>");
	}
	
	function cellSize(minutes) {
	    dps1.commandCallBack("scale", { cellDuration: minutes });
	}

</script>

<div class="note"><b>Note:</b> Read more about the <a href="http://doc.daypilot.org/scheduler/scale/">scheduler scale</a> [doc.daypilot.org].</div>
