...
public partial class Scheduler_TimeHeaderActiveAreas : System.Web.UI.Page
{
    ...
    protected void DayPilotScheduler1_BeforeResHeaderRender(object sender, DayPilot.Web.Ui.Events.Scheduler.BeforeResHeaderRenderEventArgs e)
    {
            ...
            e.Areas.Add(new Area().Width(17).Bottom(0).Right(0).Top(0).CssClass("resource_action_menu").Html("<div><div></div></div>").ContextMenu("cmRes"));
    }
    ...
    protected void DayPilotScheduler1_BeforeTimeHeaderRender(object sender, DayPilot.Web.Ui.Events.BeforeTimeHeaderRenderEventArgs e)
    {
        if (e.Level == 0)
        {
            DateTime point1 = e.Start.AddDays(10);
            DateTime point2 = e.Start.AddDays(20);

            e.Areas.Add(new Area().Top(0).Bottom(0).Start(e.Start).End(point1).Style("box-sizing: border-box; border-right: 1px solid #ccc").Html("1/3 of " + e.InnerHTML).Visible());
            e.Areas.Add(new Area().Top(0).Bottom(0).Start(point1).End(point2).Style("box-sizing: border-box; border-right: 1px solid #ccc").Html("2/3 of " + e.InnerHTML).Visible());
            e.Areas.Add(new Area().Top(0).Bottom(0).Start(point2).End(e.End).Style("box-sizing: border-box; border-right: 1px solid #ccc").Html("3/3 of " + e.InnerHTML).Visible());
